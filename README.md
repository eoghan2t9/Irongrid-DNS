...
| `dnssec` | `enabled` sets the DO bit upstream; `require_ad` rejects an answer without the upstream's AD bit as SERVFAIL (trusts an *encrypted* validating upstream); `validate_locally` goes further with real local chain-of-trust validation (RRSIG/DNSKEY/DS verified up to a trust anchor fetched from IANA) — catches a compromised/lying upstream, opportunistic on signed zones, fails open on an infrastructure hiccup, off by default |
